...
void main() {
  ...
  float progress1 = clamp((uTime - uStartTime) / max(2.0, 0.0001), 0.0, 1.0);
  progress1 = smoothstep(0.0, 1.0,  uTriggered * progress1);

  float progress2 = clamp((uTime - (uStartTime + 0.1)) / max(2.4, 0.0001), 0.0, 1.0);
  progress2 = smoothstep(0.0, 1.0,  uTriggered * progress2);  

  float progress3 = clamp((uTime - (uStartTime + 0.2 )) / max(2.3, 0.0001), 0.0, 1.0);
  progress3 = smoothstep(0.0, 1.0,  uTriggered * progress3);
