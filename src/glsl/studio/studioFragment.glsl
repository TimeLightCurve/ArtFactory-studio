...
void main() {
  ...
  float enter = uTriggered * progress;

  // Corners first, image after
  float sCorners = mix(0.0, 1.0, smoothstep(-0.2, 0.85, enter));
  float sImage   = mix(0.0, 0.9, smoothstep(0.20, 1.00, enter));
  float eImage   = mix(0.0, 1.0, smoothstep(0.30, 1.00, enter));
  ...
  img.a = mix(0.9, 0.0, 1.0 - rectAA(uv, vec2(0.02), vec2(0.98), 0.0));
  vec4 color = mix(img, vec4(vec3(0.1), 0.9), c);
  gl_FragColor = color;
